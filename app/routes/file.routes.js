...
module.exports = (app) => {
  ...
  router.post("/:id", fetchUser, [], async (req, res) => {
    ...
    for (const f in req.files) {
      ...
      let filePath =
        uploadPath + "/" + fileRec.id + "." + fileDetail.name.split(".").pop();
...
router.get("/lead-email/", fetchUser, async (req, res) => {
  ...
  // .......................................get all related file by parentID.....................................
  router.get("/:id/projects/plan/:documenttype", fetchUser, async (req, res) => {
    try {
      console.log("()()()())");
      File.init(req.userinfo.tenantcode);
      console.log("resultpicpro", req.params.id);
      let resultFile = await File.findByParentpic(req.params.id, req.params.documenttype);
  ...
  router.put("/:id", fetchUser, async (req, res) => {
    try {
      const { title, filetype, filesize, description, sectionintemplate, ispdf } = req.body;
      const errors = [];
      const fileRec = {};
