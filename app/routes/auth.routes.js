...
module.exports = app => {
  ...
  async (req, res)=>{
    ...
    const {firstname, lastname, email,joiningdate ,leavingdate , password, userrole} = req.body;
  ...
  //   const findAllRes = await Auth.findAll("b360b650-72df-4f97-8fcd-0f71a4dc1cd9");
  ...
  router.put("/:id/profile", fetchUser, async (req, res) => {
    ...
    if (resultFile) {
      ...
      for (const value of resultFile) {
        ...
        const filePath = `${process.env.FILE_UPLOAD_PATH}${req.userinfo.tenantcode}/users/${parentId}.${fileType}`;
...
router.delete("/myimage", fetchUser, async (req, res) => {
  try {
    ...
    if (!parentId) {
      return res.status(400).json({ "success": false, "message": "Missing parentid in the request body" });
    }

    const resultFile = await File.findByParentId(parentId);

    if (!resultFile || resultFile.length === 0) {
      return res.status(400).json({ "success": false, "message": "No files found for the given parentid" });
    }
    ...
    for (const value of resultFile) {
      ...
      if (fs.existsSync(filePath)) {
        ...
        if (!result) {
          return res.status(400).json({ "success": false, "message": "Failed to delete file record" });
        }
  ...
  router.put("/:id", fetchUser,  async (req, res)=>{
    try {
      ...
      const {firstname, lastname, email,joiningdate,leavingdate, phone, userrole, password, isactive, managerid } = req.body;
      const errors = [];
      const userRec = {};
      ...
      if(req.body.hasOwnProperty("joiningdate")){userRec.joiningdate = joiningdate}; 
      if(req.body.hasOwnProperty("leavingdate")){userRec.leavingdate = leavingdate};
  ...
  // Fetch all Users
  router.get("/active-users", fetchUser,  
...
router.get("/myimage", fetchUser, async (req, res) => {
  try {
      ...
      //const filePath = "D:/Files/" + parentId +"/"+ fileId + '.' + fileType;
        ...
        let filePath = process.env.FILE_UPLOAD_PATH + req.userinfo.tenantcode + "/users/" + req.userinfo.id + "." + resultFile[0].filetype ;
